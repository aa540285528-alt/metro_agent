...
**架构：** Chroma 是知识索引卷唯一持有者，仅通过 HTTP 访问。indexer 校验并用单条 registry upsert 发布 release descriptor；read-proxy 在转发应用请求前验证 descriptor，且只允许当前版本读取。`app` 除 `app_backend` 外加入 `knowledge_frontend` 与内部 `app_egress`；Squid 作为唯一接入外部网络的服务，按 allowlist 连接 `controlled_egress`。`app` 绝不加入 `knowledge_backend`。Redis 串行化发布操作，不可变 artifact 事件保存审计证据。
...
def test_compose_keeps_app_off_the_chroma_backend():
    ...
    assert {"knowledge_frontend", "app_egress"}.issubset(services["app"]["networks"])
