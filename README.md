...
4. 首次且仅首次，交互式写入第一个管理员：
   ...
   ```bash
   docker compose exec app python -m metro_agent.auth.bootstrap_admin --username admin
   ```

   密码不会作为命令行参数保存。并发执行时 MySQL 命名锁保证最终只创建一个首管理员。
...
写接口要求有效的管理员 Cookie 和精确同源 `Origin`。不允许管理员自降级或自禁用，也不允许禁用或降级最后一个活动管理员。操作会写入 MySQL 审计表；密码使用 Argon2id 哈希，原始会话令牌不落库。
...
| Chroma/知识索引 | 已发布知识切片与索引；需另行执行知识权限和发布治理。 |

服务端从 Cookie 解析身份，业务归属使用 `owner_id = auth:<id>`。LangGraph checkpoint 使用带身份命名空间的 `auth:<id>:<thread_id>`，客户端不能提交 `user_id` 来改变归属。监控接口只允许管理员访问。

## Legacy Owner 显式迁移

旧版本可能使用自由字符串或裸数字 owner。新账号**绝不自动继承裸数字 owner**，即使旧值刚好等于新的 MySQL 自增 ID。每个旧身份必须由管理员确认后显式映射。

1. 停止写流量，备份 PostgreSQL 与 Redis，并记录两个 Alembic revision 和镜像 digest。
2. 在 MySQL 管理视图确认目标账号 ID，形成经审批的 `legacy_owner -> auth:<id>` 映射表；一名旧用户只能映射到一个新账号。
...
6. Redis checkpoint 是短期状态。排空旧请求后，仅删除已确认映射用户的旧 checkpoint，要求用户从已迁移的 PostgreSQL 历史开启新轮次；不要猜测或批量改写 Redis 内部键格式。
7. 验证目标用户可见、其他用户得到 `404`，保存 SQL、审批人、备份位置、行数与回滚结果。

## 运维基线

- **健康与迁移**：监控 `/api/health`、容器健康状态和两个一次性迁移作业；每次发布记录主库与身份库 revision。
- **备份恢复**：分别备份 MySQL、PostgreSQL 和知识索引；Redis checkpoint 可按短期状态重建。恢复演练必须在隔离环境完成并记录 RPO/RTO。
