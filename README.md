...
公开仓库仅包含源码、确定性测试和部署配置；不包含生产密钥、真实运维记录、用户会话数据或真实知识索引。

## 界面展示

![Metro Agent 对话首页：左侧为新对话、监控页面和知识管理入口；中间为模型选择、问题输入、深度思考、联网搜索与附件入口。](assets/metro-agent-home.png)

截图展示了系统的对话首页：

- 左侧边栏提供**开启新对话**、**监控页面**和**知识管理**入口；知识管理仅向管理员开放。
- 中央工作区提供模型选择（截图中为 `DeepSeek V4-pro`）、自然语言输入、深度思考、联网搜索和附件入口。
- 输入框下方提供“写一段 Python 脚本”和“总结这篇文章”等快捷示例，帮助用户快速发起任务。
